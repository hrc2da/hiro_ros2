...
class ArmPose:
    ...
    def getServoElevator(self):
        ...
        return 45-degrees(self.shoulder_angle)
    def getServoActuator(self):
        # return degrees(self.actuator_angle) + 204.78
        # return 180 - degrees(self.actuator_angle)
        # return 180-degrees(self.elbow_angle)
        return 45-degrees(self.actuator_angle)
...
class ArmController:
    ...
    def getIKPose(self):
        if self.ik_dirty and self.ik.valid:
            ...
            main_arm = self.ik.elbow - self.ik.originpl # vector from origin to elbow
            arm_vert_angle = sigangle(main_arm, vertical) # angle between main arm and vertical (0,1)
            forearm = self.ik.wristpl - self.ik.elbow #vector from elbow to wrist
